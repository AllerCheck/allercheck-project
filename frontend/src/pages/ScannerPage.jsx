...
const ScannerPage = () => {
  ...
  useEffect(() => {
    if (!isRegistered && scanCount >= 5) {
      alert(
        "Tageslimit erreicht! Bitte registriere dich für unbegrenzte Scans."
      );
      navigate("/register");
    }
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-r from-gray-900 via-gray-800 to-gray-700 p-4">
        ...
        <div className="mt-6 bg-white p-6 rounded-lg shadow-md w-full max-w-md text-center">
          ...
          {productData.image && (
            <img
              ...
              alt="Produktbild"
              className="mt-4 w-80 mx-auto rounded"
            />
          )}
          <h4 className="mt-4 font-bold text-2xl text-yellow-700">Ingredients:</h4>
          <p className="text-gray-600">
            {productData.ingredients || "No ingredients found"}
          </p>
          <h4 className="mt-4 font-bold text-2xl text-yellow-700">Allergies:</h4>
          <ul className="text-gray-600">
            {productData.allergens?.length ? (
              productData.allergens.map((allergen, index) => (
                <li key={index} className="text-xl font-bold">
                  {allergen.split(":")[1]?.trim() || allergen.trim()}
                </li>
              ))
            ) : (
      ...
      {!isRegistered && scanCount >= 5 && (
        <div className="mt-6 bg-red-100 border border-red-400 text-red-700 p-4 rounded w-full max-w-md text-center">
          <p>
            Du hast dein Tageslimit erreicht. Registriere dich für unbegrenzte
            Scans!
          </p>
          <button
